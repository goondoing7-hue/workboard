...
import { ArrowDownToLine, ArrowLeft, ArrowUpFromLine, BarChart3, CalendarDays, Check, CheckCircle2, ChevronRight, ClipboardCheck, Cloud, CloudOff, Database, FileCheck2, FileText, HardDrive, Info, List, Loader2, LockKeyhole, Plus, RefreshCw, Search, Settings2, ShieldCheck, Trash2, TriangleAlert, Users, X } from 'lucide-react';
...
const MOBILE_NAV = { overview: '실적', records: '기록', recognition: '인정', military: '병영', settings: '백업' };
const TARGETS = [
  { id: 'kcp', label: '한상심', full: '한국상담심리학회', authority: '수퍼바이저', channel: 'supervisor' },
  { id: 'kca', label: '한상', full: '한국상담학회', authority: '센터', channel: 'center' },
...
function RecognitionFields({ channel, value, onChange }) {
  const title = channel === 'center' ? '센터 확인 · 한상 / 병영' : '수퍼바이저 확인 · 한상심';
  ...
  return <fieldset className="perf-recognition-fields"><legend>{title}</legend><div className="perf-form-grid">
    ...
    <Field label="확인자"><input value={value.approver || ''} placeholder={channel === 'center' ? '기관 또는 담당자' : '수퍼바이저'} onChange={event => patch('approver', event.target.value)} maxLength={200} /></Field>
...
function RecordDialog({ record, onSave, onClose, onDelete }) {
    ...
    <form onSubmit={save}>
      ...
      <div className="perf-dialog-body">
        ...
        <section className="perf-form-section"><h3>횟수 · 인원 · 시간</h3><div className="perf-form-grid perf-form-three">
          ...
          {draft.activity === 'supervision' ? <Field label="수퍼바이저"><input value={draft.supervisor || ''} onChange={event => set('supervisor', event.target.value)} maxLength={200} /></Field> : null}
        </section>
        <section className="perf-form-section"><h3>인정 대상과 확인</h3><div className="perf-target-checks">{TARGETS.map(target => <label className="perf-checkbox" key={target.id}><input type="checkbox" checked={Boolean(draft.targets?.[target.id])} onChange={event => set('targets', { ...draft.targets, [target.id]: event.target.checked })} /><span>{target.label}</span></label>)}</div><p className="perf-field-help">센터 확인은 한상과 병영에 함께 적용됩니다. 한상심은 수퍼바이저 확인을 별도로 관리합니다. 최종 자격 인정 기준은 제출처의 심사를 따릅니다.</p>
          ...
          {draft.targets?.kcp ? <RecognitionFields channel="supervisor" value={draft.recognition?.supervisor || { status: 'pending' }} onChange={value => set('recognition', { ...draft.recognition, supervisor: value })} /> : null}
        </section>
...
function ConflictList({ store, runAction }) {
  ...
  return <section className="perf-panel perf-conflict-panel"><div className="perf-panel-title"><div><h2>확인이 필요한 변경 {num(store.conflicts.length)}건</h2><p>같은 기록에 여러 변경이 있습니다. 내용을 확인하고 사용할 버전을 선택하세요.</p></div><TriangleAlert size={23} /></div><div className="perf-panel-body">{store.conflicts.map(conflict => <div className="perf-conflict" key={conflict.entityId}><strong>{conflict.versions?.[0]?.payload?.caseId || '기록 변경 충돌'}</strong><div className="perf-conflict-versions">{conflict.versions.map(version => <div key={version.id || version.eventId}><div><strong>{version.payload?.date || version.payload?.name || '저장된 변경'}</strong><span>{version.payload?.activity ? activityLabel(version.payload.activity) : '자격요건 정보'} · {dateTime(version.createdAt || version.timestamp)}</span>{version.payload?.sessions ? <span>{num(version.payload.sessions)}회 · {duration(version.payload.minutes)}</span> : null}</div><button className="perf-button" onClick={() => runAction(() => store.resolveConflict(conflict.entityId, version.id || version.eventId))}>이 버전 사용</button></div>)}</div></div>)}</div></section>;
}
...
function PerformanceApp() {
  ...
  const targetSummaries = useMemo(() => TARGETS.map(item => ({ ...item, summary: summarize(records, { ...range, target: item.id }), eligible: periodRecords.filter(record => record.status === 'done' && record.targets?.[item.id]) })), [records, range.from, range.to, periodRecords]);
  ...
  const performedSummary = useMemo(() => summarize(records, { ...range, target: 'performed' }), [records, range.from, range.to]);
  return <div className="perf-app">
    <aside className="perf-sidebar"><a href="/performance.html" className="perf-brand" aria-label="상담실적 첫 화면"><span><ClipboardCheck size={25} /></span><div><strong>상담실적</strong><small>기록과 인정 관리</small></div></a><div className="perf-workspace-label">나의 상담 기록</div><nav aria-label="실적관리 메뉴">{NAV.map(item => <button key={item.id} className={tab === item.id ? 'active' : ''} onClick={() => setTab(item.id)} aria-label={item.title} title={item.title} data-mobile-label={MOBILE_NAV[item.id]} aria-current={tab === item.id ? 'page' : undefined}><item.icon size={19} /><span>{item.title}</span>{item.id === 'recognition' && pending.length ? <b>{pending.length}</b> : null}</button>)}</nav><div className="perf-sidebar-bottom"><button className={`perf-backup-indicator ${storageError || sync.error ? 'has-error' : ''}`} onClick={() => setTab('settings')} aria-label={`${backupState} · 백업 설정`}><BackupIcon className={sync.busy ? 'perf-spin' : ''} size={19} /><span><strong>{backupState}</strong><small>{sync.lastSynced ? `최근 백업 ${dateTime(sync.lastSynced)}` : '백업 · 연결 확인'}</small></span><ChevronRight size={15} /></button><a className="perf-back-link" href="/"><ArrowLeft size={15} />업무보드로 이동</a></div></aside>
    <div className="perf-main"><header className="perf-topbar"><div className="perf-breadcrumb">상담실적<span>/</span><strong>{currentNav.title}</strong></div><span className="perf-topbar-date"><CalendarDays size={16} />{new Date().toLocaleDateString('ko-KR', { year: 'numeric', month: 'long', day: 'numeric', weekday: 'short' })}</span></header>
      <main className="perf-content"><div className="perf-page-heading"><div><h1>{currentNav.title}</h1><p>{tab === 'overview' ? '한 번 기록하고, 진행 실적과 인정 실적을 함께 확인하세요.' : tab === 'records' ? '마음결 사업의 상담도 개인상담에 함께 집계합니다.' : tab === 'recognition' ? '센터 확인과 수퍼바이저 확인을 나누어 관리합니다.' : tab === 'military' ? '경력과 상담 실적, 제출에 필요한 증빙을 함께 정리합니다.' : '내 기록의 저장 위치와 백업 상태를 확인하세요.'}</p></div>{tab !== 'settings' ? <button className="perf-button perf-primary" onClick={() => setEditing({ new: true })}><Plus size={18} />기록 추가</button> : null}</div>
        ...
        {tab === 'recognition' ? <><div className="perf-note"><Info size={18} /><span>총 {num(performedSummary.sessions)}회 진행 · 아래 합계는 선택한 대상의 <strong>확인 완료 기록</strong>입니다. 한상과 병영은 센터 확인을 공유하며, 각 제출처의 자격요건은 별도로 적용됩니다.</span></div><div className="perf-recognition-grid">{targetSummaries.map(item => { const missing = item.eligible.filter(record => channelStatus(record, item.channel) !== 'approved'); return <section className="perf-panel perf-recognition-card" key={item.id}><div className="perf-recognition-card-head"><span className="perf-authority-tag">{item.authority} 확인</span><h2>{item.label}</h2>{item.label !== item.full ? <p>{item.full}</p> : <p>경력 · 상담 실적 관리</p>}</div><strong>{num(item.summary.sessions)}<small>회 인정 완료</small></strong><div className="perf-recognition-metrics"><span>{num(item.summary.people)}명</span><span>{duration(item.summary.minutes)}</span></div><div className="perf-recognition-progress"><span style={{ width: `${item.eligible.length ? (item.eligible.length - missing.length) / item.eligible.length * 100 : 0}%` }} /></div><div className="perf-recognition-bottom"><span>확인 대기 <strong>{num(missing.length)}건</strong></span><button className="perf-text-button" onClick={() => { setTab('records'); setTarget(item.id); setStatus('all'); setActivity(''); setSearch(''); }}>인정 기록 보기<ChevronRight size={14} /></button></div></section>; })}</div><section className="perf-panel"><div className="perf-panel-title"><div><h2>확인 대기 기록 <span className="perf-count">{num(pending.length)}</span></h2><p>확인 일자, 확인자, 증빙 위치를 기록하고 확인 완료로 변경하세요.</p></div></div>{pending.length ? <RecordTable records={pending} onEdit={setEditing} /> : <Empty title="확인 대기 기록이 없습니다" description="인정 대상이 있는 진행 완료 기록 중 확인이 남은 항목이 여기에 표시됩니다." />}</section></> : null}
    ...
    {editing ? <RecordDialog key={editing.id || 'new'} record={editing.new ? null : editing} onClose={() => setEditing(null)} onSave={async record => { const result = await store.saveRecord(record); if (result === false) return false; const recheck = ['center', 'supervisor'].some(channel => record.recognition?.[channel]?.status === 'approved' && result?.recognition?.[channel]?.status === 'requested'); setMessage({ text: recheck ? '기록을 저장했습니다. 실적 변경으로 기존 인정은 재확인이 필요합니다.' : '기록을 저장했습니다.' }); return result; }} onDelete={deleteRecord} /> : null}
  </div>;
}
